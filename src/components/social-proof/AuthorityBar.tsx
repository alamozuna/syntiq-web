...
export default function AuthorityBar() {
  return (
    <section className="relative py-16 sm:py-24 border-y border-slate-800/60 bg-slate-950/70 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section Overline */}
        <div className="text-center mb-12">
          <span className="brand-label text-slate-400">
            RESULTADOS OPERATIVOS MEDIBLES & AUDITABLES
          </span>
          <h2 className="font-brand-display text-3xl sm:text-4xl text-white font-light mt-2">
          ...
          {STATS.map((stat, i) => {
            ...
            return (
              <motion.div
                ...
                className="glass-panel p-6 rounded-2xl border border-slate-800 hover:border-blue-500/40 transition-all group"
              >
                <div className="flex items-center justify-between mb-4">
                  <span className="text-3xl sm:text-4xl font-light font-brand-display text-white tracking-tight group-hover:text-blue-400 transition-colors">
                    {stat.value}
                  </span>
                  <div className="p-2.5 rounded-xl bg-blue-950/40 border border-blue-500/20 text-blue-400 group-hover:scale-110 transition-transform">
                    <Icon className="w-5 h-5" />
                  </div>
                </div>
                <h4 className="text-sm font-semibold text-slate-200 mb-1">{stat.label}</h4>
        ...
        <div className="pt-10 border-t border-slate-800/80">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
            {ECOSYSTEM_TOOLS.map((tool) => (
              <div
                key={tool.name}
                className="p-3 rounded-xl bg-slate-900/50 border border-slate-800 hover:border-slate-700 flex flex-col items-center justify-center text-center transition-all hover:bg-slate-900 group"
              >
                <span className="text-[11px] font-medium text-slate-200 group-hover:text-blue-400 transition-colors">
                  {tool.name}
                </span>
